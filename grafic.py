...
class Grafics:
    ...
    def __init__(self, data) -> None:
        ...
        self.df = pd.DataFrame(
            data={'margin': data[1], 'clients': data[3]}, index=range(1, data[0]+1))

    def circular(self, expenses):

        dt = pd.Series(data=[self.tax[-1], expenses, self.money[-1]],
                       index=['налог', 'расходы', 'маржа'])
...
if __name__ == '__main__':
    ...
    geo_data = Calculable.progression(mode='G')#type: ignore
